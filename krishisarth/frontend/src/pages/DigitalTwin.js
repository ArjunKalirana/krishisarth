...
/**
 * KrishiSarth Elite Digital Twin (v4.0 - R3F)
 * High-fidelity real-time 3D field simulation.
 */
...
function Model({ mode, onSelect, selectedId, deviceStates }) {
  ...
  useEffect(() => {
    scene.traverse((child) => {
      if (child.isMesh) {
        ...
        // Find if this mesh is part of a labeled group
  ...
  useFrame((state, delta) => {
    // Animate active pipes or mixers if pump is on
    scene.traverse((child) => {
  ...
  return (
    <div className="w-full h-screen relative bg-[#0a0f12]">
      ...
      <Canvas shadows camera={{ position: [40, 40, 40], fov: 35 }}>
        <Suspense fallback={<Html center><div className="text-emerald-500 font-mono uppercase tracking-widest animate-pulse">Neural Linking...</div></Html>}>
